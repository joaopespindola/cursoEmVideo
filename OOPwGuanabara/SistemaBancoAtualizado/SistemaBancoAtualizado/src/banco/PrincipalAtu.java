...
public class PrincipalAtu {
    ...
    static ClienteAtu[] clientes = new ClienteAtu[10];
    
    static ClienteAtu logar(Scanner sc) {
        System.out.print("Usuario: ");
        String usr = sc.nextLine();
        System.out.print("Senha: ");
        String sen = sc.nextLine();
        for (int i=0;i<ClienteAtu.getCt();i++)
            if ((clientes[i].getUsr().equals(usr))&&(clientes[i].getSen().equals(sen)))
                return clientes[i];
        return null;
    }
    
    static ContaAtu buscaConta(int num) {
        for (int i=0;i<ClienteAtu.getCt();i++)
        ...
        while (op != 9) {
            ...
            switch(op) {
                case 1:
                    ...
                    ContaAtu c1 = new ContaAtu();
                    c1.cadastraConta(tipo, saldo);
                    
                    ClienteAtu cli1 = new ClienteAtu();
